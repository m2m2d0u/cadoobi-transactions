...
@AllArgsConstructor
@Builder
@Schema(description = "Request to update an existing webhook configuration")
public class UpdateWebhookConfigurationRequest {

    @Size(max = 100, message = "Name must not exceed 100 characters")
    @Schema(description = "Webhook name for identification", example = "Payment Notifications")
    private String name;
    ...
    @Schema(description = "Maximum number of retry attempts for failed deliveries", example = "3")
    private Integer maxRetries;

    @Min(value = 5, message = "Timeout must be at least 5 seconds")
    @Max(value = 120, message = "Timeout cannot exceed 120 seconds")
    @Schema(description = "Timeout in seconds for webhook HTTP requests", example = "30")
